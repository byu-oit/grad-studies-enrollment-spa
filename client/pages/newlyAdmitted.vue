        ...
        <div v-if="$byu.user">
            <div v-if="studentList">
                <div id='student-list-header' style="border: 1px black solid; padding: 5px; margin-bottom: 15px; background: #eee; justify-content: space-between; align-items: center" class="tabular tabular-wrap">
                    <div style="margin: 0 10px">There are currently <b>{{_formattedNumber(studentList.length)}}</b> newly admitted students seeking graduate degrees</div>
...
<script>
    ...
    export default {
        ...
        computed: {
            studentList() {
                return this.$store.state.newlyAdmittedStudentList
            },
            loading() {
        ...
        mounted() {
            if (!this.studentList) {
                this.$store.dispatch('fetchStudentList', 'newlyAdmitted')
            }
