    ...
    >
      ...
      <div class="t-header">
        <div ref="tHeaderTable">
          <template v-if="!enableMultiHeader">
            <div class="header-line" :style="headerFooterStyle">
              <div
                class="header-cell"
              ...
              >
                <template v-if="item.prop === '_action'">
                  <div
                    ...
                    :style="{height: (itemHeight-12)+'px', 'justify-content': item.alignItems||'center'}"
                ...
                <template v-else>
                  ...
                  <div
                    ...
                    v-else-if="item.eTip"
                    :style="{'justify-content': item.alignItems||'center'}"
                  >
                    <base-tooltip>
      ...
      <div class="t-bottom" ref="tBottom" v-show="showSummary">
        <div ref="tBottomTable">
          <div class="bottom-line" :style="headerFooterStyle">
            <div
              class="bottom-cell"
              v-for="(item, configIndex) in configTemp.filter(v=>!v.isHidden)"
              :key="configIndex"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      lineStyle() {
        if (this.fontSize !== '') {
          return 'height: ' + this.itemHeight + 'px; font-size: '+ this.fontSize
        }
        else {
    ...
    methods: {
      ...
      handleClickItem(item) {
        ...
        if (!this.multiSelect) {
          // unselect all selected rows in the dataInitTemp array
          for (let i = 0; i < self.dataInitTemp.length; i++) {
            if (self.dataInitTemp[i]._eSelected === true) {
              self.dataInitTemp[i]._eSelected = false
              self.dataTemp[i]._eSelected = false
            }
          }
          // unselect all selected rows in the dataTemp array
          for (let i = 0; i < self.dataTemp.length; i++) {
            if (self.dataTemp[i]._eSelected === true) {
              self.dataTemp[i]._eSelected = false
            }
          }
        }
